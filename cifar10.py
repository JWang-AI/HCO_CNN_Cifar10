...
def test_input():
    """Construct test input for CIFAR training using the Reader ops.
        ...
        Returns:
          images: Images. 4D tensor of [total_num_size, IMAGE_SIZE, IMAGE_SIZE, 3] size.
          labels: Labels. 1D tensor of [total_num_size] size.
...
def inference_op(images, idv):
    ...
    with tf.variable_scope('conv1') as scope:
        kernel = _variable_with_weight_decay('weights',
                                             shape=[s_kernel_c_1, s_kernel_c_1, 3, f_map1],
                                             stddev=5e-2,
                                             wd=None)
        conv = tf.nn.conv2d(images, kernel, [1, 1, 1, 1], padding='SAME')
        biases = _variable_on_cpu('biases', [f_map1], tf.constant_initializer(0.0))
    ...
    with tf.variable_scope('conv2') as scope:
        kernel = _variable_with_weight_decay('weights',
                                             shape=[s_kernel_c_2, s_kernel_c_2, f_map1, f_map2],
                                             stddev=5e-2,
                                             wd=None)
        conv = tf.nn.conv2d(norm1, kernel, [1, 1, 1, 1], padding='SAME')
        biases = _variable_on_cpu('biases', [f_map2], tf.constant_initializer(0.1))
        pre_activation = tf.nn.bias_add(conv, biases)
